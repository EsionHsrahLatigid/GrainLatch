...
int main(int argc, char** argv)
{
    if (argc != 3)
    {
        std::cerr << "usage: GrainLatchPluginLoadTest <vst3-bundle> <expected-name>\n";
        return 2;
    }
    ...
    for (int block = 0; block < blocks; ++block)
    {
        ...
        for (int sample = 0; sample < blockSize; ++sample)
        {
            ...
            const auto value = 0.12589254f * std::sin(2.0f * juce::MathConstants<float>::pi
    ...
    if (rms <= 0.005 || peak > 0.981f || std::abs(dc) >= rms * 0.8
